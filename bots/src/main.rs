...
async fn main() -> Result<()> {
    ...
    match args.as_slice() {
        [cmd] if cmd == "status" => cmd_status().await,
        [cmd, msg @ ..] if cmd == "broadcast" => {
            let text = msg.join(" ");
            cmd_broadcast(&text).await
        }
        [cmd, sub] if cmd == "gatekeeper" && sub == "list" => cmd_gk_list().await,
        [cmd, sub, id] if cmd == "gatekeeper" && (sub == "approve" || sub == "deny") => {
            let id: i64 = id.parse().context("id must be a number")?;
            cmd_gk_resolve(id, sub == "approve").await
        }
        [cmd] if cmd == "log" => cmd_log(20).await,
...
async fn cmd_status() -> Result<()> {
    ...
    // Also query bus for live status
...
async fn cmd_broadcast(text: &str) -> Result<()> {
    if text.trim().is_empty() {
        bail!("Broadcast message cannot be empty.");
    }
    let bus = bus::BusClient::new();
    bus.broadcast(text).await.context("Failed to publish broadcast event")?;
    println!("Broadcast sent: \"{text}\"");
    Ok(())
}

async fn cmd_gk_list() -> Result<()> {
    let pool = open_db().await?;
...
async fn cmd_log(limit: i64) -> Result<()> {
    let pool = open_db().await?;
    let _ = &pool; // suppress unused warning when pool is passed by reference
