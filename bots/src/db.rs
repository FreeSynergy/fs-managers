...
impl<'a> BotDb<'a> {
    ...
    pub async fn list_instances(&self) -> Result<Vec<BotInstance>> {
    ...
    pub async fn list_pending_requests(&self) -> Result<Vec<JoinRequest>> {
        use fs_db::sea_orm::ConnectionTrait;
        let conn = self.conn.inner();
        let stmt = Statement::from_string(
            conn.get_database_backend(),
    ...
    pub async fn approve_request(&self, id: i64, approve: bool) -> Result<bool> {
        use fs_db::sea_orm::ConnectionTrait;
        let conn = self.conn.inner();
